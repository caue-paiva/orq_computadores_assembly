...
loop2:
	...
	sub char2_addr, str_size_r, i_loop_var # o endereço da str invertida será TAM - I -1
	addi char2_addr,char2_addr, -1  
	
	add char2_addr, char2_addr, str_inver #soma o offset mais o endereço base da str invertida
...
str_len:
	...
	li i_loop_var, 0 
	mv char1_addr, a1 #carrega endereço inicial da string
loop:

	lb char1_val, 0(char1_addr) #carrega o char atual
	
	addi char1_addr, char1_addr, 1 #incrementa o endereço do char (pega prox byte) e o i
	addi i_loop_var, i_loop_var, 1 
	
	beq char1_val, zero, end_loop #se o char atual tiver valor 0, ele é o \0 e o final da string
	j loop
end_loop:

	mv a0, i_loop_var #tamanho da string é o valor de i
	jr ra
